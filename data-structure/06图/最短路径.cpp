...
//表示源点到此点的最短路径已经找到.(原理：如果在下一轮时，源点到这个min点的距离还可以进行更新，即仍然有比当前轮更短的（源点到min点）的距离，那么它一定是通过当前轮的兄弟结点作为中介抵达这个min点的，而源点到兄弟结点（min点的）的最短距离已经大于源点到min点的距离，
//如果再加上兄弟结点到min点的距离更加大于前一轮中源点到min点的距离了，因此本轮找到的最小距离点就是源点到这个点的最短距离，下一轮不用管它。）找到这个点后，如果有pre数组，则将这个目标点的前驱记录在pre数组中，方便最后输出最短路径。
//上轮结束开始下一轮，通过扫描邻接矩阵中上一轮目标点的整行，更新所有未访问点的dist[],得到新一轮未访问点的dist集合，找出最小点
 //----------------------------------------------------------------------------------------
