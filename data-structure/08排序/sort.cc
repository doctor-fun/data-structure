...
//快速排序
//基本有序的情况下最慢，
//可以直接由枢轴分成几近对等的两半的快排最快
//快排的阶段性成果的特点是：完成第i趟后，会有i个以上的数出现在它最终要出现的位置
//快排每一趟结束后都会产生一个中位数
...
--------------------------------------
//归并排序
//某层递归中，准备将已各自排好序的两组a[],b[]进行归并（从小到大）
//新建一个c[n]数组，开始逐个对比a,b两个数组的元素，如a<b则将a[i++]复制到c[k]中，k++，
//如果a>b，则将b[i++]复制到c[k]中，然后k++
